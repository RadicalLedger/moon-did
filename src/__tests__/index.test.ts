...
import { MoonbeamDIDRegistry, getResolver } from 'moon-did-resolver'
import { DelegateTypes, MoonDID, KeyPair } from '../index'
...
describe('MoonDID', () => {
        ...
        describe('HubService', () => {
          ...
          it('resolves document', async () => {
            return expect((await resolver.resolve(did)).didDocument).toEqual({
              ...
              service: [
                {
                  id: 'did:moon:dev:0x7E5F4552091A69125d5DfCb7b8C2659029395Bdf#service-1',
                  type: 'HubService',
  ...
  describe('signJWT', () => {
    ...
    describe('plain vanilla key pair account', () => {
      it('should sign valid jwt', async () => {
        const kp: KeyPair = MoonDID.createKeyPair('dev')
        plainDid = new MoonDID({
          ...kp,
          provider,
          registry: registry,
        })
  ...
  describe('verifyJWT', () => {
    const ethrDidAsIssuer = new MoonDID(MoonDID.createKeyPair('dev'))
  ...
  describe('base58 key', () => {
    ...
    it('supports base58 keys as hexstring', async () => {
      ...
      const did = `did:moon:dev:${delegate1}`
      const didController = new MoonDID({
        identifier: did,
        provider,
        registry,
      })
    ...
    it('supports base58 keys as string', async () => {
      const did = `did:moon:dev:${delegate2}`
      const didController = new MoonDID({
        identifier: did,
        provider,
        registry,
...
describe('MoonDID (Meta Transactions)', () => {
  let ethrDid: MoonDID,
    walletSigner: MoonDID,
    registry: string,
