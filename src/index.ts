...
import { MoonDidController, interpretIdentifier, MetaSignature, REGISTRY } from 'moon-did-resolver'
...
export class MoonDID {
  public did: string
  public address: string
  public signer?: JWTSigner
  public alg?: 'ES256K' | 'ES256K-R'
  private owner?: string
  private readonly controller?: MoonDidController
  ...
  constructor(conf: IConfig) {
    ...
    if (conf.provider || conf.rpcUrl || conf.web3) {
      ...
      this.controller = new MoonDidController(
        conf.identifier,
        undefined,
        txSigner,
        chainNameOrId,
      ...
      this.did =
        typeof publicKey === 'string' ? `did:moon:${networkString}${publicKey}` : `did:moon:${networkString}${address}`
    }
  ...
  static createKeyPair(chainNameOrId?: string | number): KeyPair {
    ...
    const identifier = net ? `did:moon:${net}:${publicKey}` : publicKey
  ...
  async createSigningDelegate(
    ...
    const kp = MoonDID.createKeyPair()
